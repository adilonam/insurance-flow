...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!file || !bankAccountId || !startDate || !endDate) {
    ...
    // Verify the bank account belongs to this claim
    if (bankAccount.financialStep.claimId !== id) {
      return NextResponse.json({ error: "Bank account does not belong to this claim" }, { status: 403 });
    }
    ...
    const fileKey = `claims/financial/${id}/${bankAccountId}/${timestamp}-${randomId}.${fileExtension}`;
    ...
    return NextResponse.json({ error: "Failed to upload bank statement" }, { status: 500 });
  }
}

// DELETE - Delete bank statement
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (statement.bankAccount.financialStep.claimId !== id) {
