...
export function EditPartnerForm({ id }: EditPartnerFormProps) {
  ...
  // Helper to load selected ServiceProvider
  const loadSelectedServiceProvider = async (field: string, serviceProviderId: string | null) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setServiceProviderStates((prev) => ({
          ...prev,
          [field]: { ...prev[field], selected: { id: provider.id, name: provider.name, type: provider.type } },
        }));
      }
    } catch (error) {
      console.error(`Error loading service provider for ${field}:`, error);
    }
  };
    ...
    const fetchPartner = async () => {
      try {
        ...
        form.reset({
          ...
          vehicleRecoveryId: data.vehicleRecoveryId || "",
          vehicleStorageId: data.vehicleStorageId || "",
          replacementHireId: data.replacementHireId || "",
          vehicleRepairsId: data.vehicleRepairsId || "",
          independentEngineerId: data.independentEngineerId || "",
          vehicleInspectionId: data.vehicleInspectionId || "",
        });

        // Load selected ServiceProviders
        await Promise.all([
          loadSelectedServiceProvider("vehicleRecovery", data.vehicleRecoveryId),
          loadSelectedServiceProvider("vehicleStorage", data.vehicleStorageId),
          loadSelectedServiceProvider("replacementHire", data.replacementHireId),
          loadSelectedServiceProvider("vehicleRepairs", data.vehicleRepairsId),
          loadSelectedServiceProvider("independentEngineer", data.independentEngineerId),
          loadSelectedServiceProvider("vehicleInspection", data.vehicleInspectionId),
        ]);
      } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-8">
        ...
        <span className="text-muted-foreground ml-2">Loading partner...</span>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the information for this partner</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
